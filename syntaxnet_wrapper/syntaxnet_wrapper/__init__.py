...
#print('syntaxnet_wrapper pwd:', pwd)
...
syntax_root='/opt/tensorflow/syntaxnet'
...
class SyntaxNetWrapper(object):
    ...
    def start(self):
      try:  
        rundir = join(syntax_root, 'bazel-bin/syntaxnet/parser_eval.runfiles/__main__')
        tf_dir= join(syntax_root, 'bazel-bin/syntaxnet/parser_eval.runfiles/__main__/external/org_tensorflow')

        command = ['python', self.run_filename, self.model_path, self.context_path]

        env = os.environ.copy()
        env['PYTHONPATH'] = ":".join([rundir, tf_dir])
    ...
    def __init__(self, run_filename, model_name):
        ...
        if model_name == 'English-Parsey':
            model_path = 'syntaxnet/models/parsey_mcparseface'
            context_path = 'syntaxnet/models/parsey_mcparseface/context.pbtxt'
        elif model_name == 'ZHTokenizer':
            model_path = '/mnt/ud_models/Chinese'
            context_path = 'syntaxnet/models/parsey_universal/context-tokenize-zh.pbtxt'
        else:
            model_path = '/mnt/ud_models/{!s}'.format(model_name)
            context_path = 'syntaxnet/models/parsey_universal/context.pbtxt'

        context_path = join(syntax_root, context_path)
        if not model_path.startswith('/') : 
            model_path = join(syntax_root, model_path)
    ...
    def list_models(self):
        #pwd = dirname(abspath(__file__))
        #model_path = join(pwd, 'syntaxnet/models/parsey_universal')
        model_path='/mnt/ud_models'
        files = os.listdir(model_path)
        models = []
        for fn in files:
            if os.path.isdir(os.path.join(model_path, fn)):
                models.append(fn)
        models.append(join(syntax_root,'syntaxnet/models/parsey_mcparseface'))
